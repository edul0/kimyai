...
class LLMRouter:
    ...
    def _system_prompt(self, provider: str, mode: str) -> str:
        ...
        return (
            ...
            "LOGIN MODERNO + DEFESA DE DADOS (obrigatorio quando houver login): visual 2025 (split-screen com painel de marca OU card glassmorphism centralizado; logo em SVG monograma, nunca emoji; olhinho de senha em SVG). Seguranca client-side DURA: senha NUNCA em texto puro (nem localStorage nem variavel) — só hash SHA-256 com salt via crypto.subtle.digest; sessao = token aleatorio de crypto.getRandomValues (nunca e-mail/senha); bloqueio de 5 tentativas erradas por 30s com contagem visivel; mensagem de erro GENERICA ('E-mail ou senha incorretos' — nunca dizer qual campo errou); todo dado de usuario renderizado com textContent/escape (anti-XSS). Comente no codigo que producao exige backend com hash no servidor. "
            "PROFUNDIDADE MINIMA (sistema raso = reprovado): cada modulo com criar+editar+excluir+busca+filtro e 8+ registros realistas; dashboard com KPIs CALCULADOS dos dados + 1 grafico; exportar CSV; mascaras pt-BR (moeda/telefone/data); modulos RELACIONADOS entre si (ex.: venda baixa estoque, OS consome peca) — e isso que separa sistema de verdade de brinquedo. "
