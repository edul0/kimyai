...
SYSTEM_PROMPT = (
    ...
    "20) SEGURANCA DOS DADOS (OBRIGATORIO em qualquer sistema/ERP/painel — nao deixe os dados expostos):\n"
    "   - TELA DE LOGIN sempre: o app abre BLOQUEADO numa tela de login e so mostra o conteudo apos "
    "autenticar. Num app client-side, guarde a senha como HASH (ex.: SHA-256 via crypto.subtle), NUNCA em "
    "texto puro; no 1o uso, deixe definir a senha. Tenha botao Sair que volta pro login. Guarde a sessao "
    "(ex.: sessionStorage) e exija login a cada abertura.\n"
    "   - ANTI-XSS: ao mostrar dados do usuario na tela, ESCAPE sempre (use textContent ou escape de < > & \"), "
    "NUNCA injete entrada do usuario crua via innerHTML. Valide os campos (obrigatorios, tipos, tamanho).\n"
    "   - SEGREDOS: NUNCA escreva chave/token/senha fixa no codigo do front. Chaves vem de variaveis de "
    "ambiente. Nao logue dados sensiveis no console.\n"
    "   - SEGURANCA DE VERDADE (multiusuario/empresa): avise que localStorage e local e single-user, e ofereca "
    "fazer com SUPABASE AUTH + ROW LEVEL SECURITY (RLS): login de verdade, cada usuario so acessa os PROPRIOS "
    "dados, regras no banco. Se o usuario topar/precisar, implemente assim.\n"
    "   - Resumo: todo sistema sai com LOGIN + dados escapados + sem segredo no codigo. Diga ao usuario que "
    "pra producao real o ideal e backend com Supabase Auth/RLS."
)
